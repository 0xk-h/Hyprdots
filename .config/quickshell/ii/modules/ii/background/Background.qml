...
Variants {
    ...
    PanelWindow {
        ...
        property real minSuitableScale: 1
        ...
        onWallpaperPathChanged: {
            ...
            if (!bgRoot.wallpaperSafetyTriggered && bgRoot.wallpaperPath !== "") {
                awwwSetWallpaper.setWallpaper(bgRoot.wallpaperPath);
            }
        }
        ...
        Process {
            ...
            function setWallpaper(path) {
                awwwSetWallpaper.running = false;
                awwwSetWallpaper.command = [
                    "awww", "img", path,
                    "--transition-type", "any",
                    "--transition-duration", "1",
                    "--transition-fps", "60"
                ];
                awwwSetWallpaper.running = true;
            }

            Component.onCompleted: Qt.callLater(() => {
                const path = bgRoot.wallpaperSafetyTriggered ? "" : bgRoot.wallpaperPath;
                if (path !== "") setWallpaper(path);
            })
        }

        // Wallpaper zoom scale (Broken)
        ...
        Item {
            ...
            // Lock screen blur — sources from a Rectangle since awww owns the actual image
            Loader {
                id: blurLoader
            ...
            // Invisible source for blur — just a solid color overlay that blurLoader can sample
            Rectangle {
                id: blurSource
                anchors.fill: parent
            ...
            WidgetCanvas {
                ...
                // Without a Image element, base offset is always 0
                readonly property real baseWallpaperOffsetX: 0
                readonly property real baseWallpaperOffsetY: 0
                readonly property real wallpaperTotalOffsetX: -bgRoot.parallaxTotalPixelsX * wallpaperUsedFractionX
                readonly property real wallpaperTotalOffsetY: -bgRoot.parallaxTotalPixelsY * wallpaperUsedFractionY
                readonly property bool locked: GlobalStates.screenLocked

                // Should add the parallax in future
                ...
                readonly property real wallpaperUsedFractionX: {
                ...
                readonly property real wallpaperUsedFractionY: {
                    let usedFraction = middleFraction;
                    if (Config.options.background.parallax.enableWorkspace && bgRoot.verticalParallax)
                        usedFraction = fraction;
                    return Math.max(0, Math.min(1, usedFraction));
